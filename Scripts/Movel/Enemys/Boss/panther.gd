...
func setup(_beast_manager : BeastsManager, _mar_ambush_r : Marker2D, _mar_ambush_l : Marker2D, _mar_relax_r : Marker2D, _mar_relax_l : Marker2D) -> void:
	...
	var ambush_l = self.mar_ambush_l.global_position - self.global_position
	var ambush_r = self.mar_ambush_r.global_position - self.global_position
	self.hand_l.setup(ambush_l, relax_l)
	self.hand_r.setup(ambush_r, relax_r)
